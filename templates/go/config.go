// Package applink is a Go port of templates/typescript.
//
...
func LoadConfig() (*Config, error) {
	appID, err := requireEnv("APPLINK_APP_ID")
	if err != nil {
		return nil, err
	}
	password, err := requireEnv("APPLINK_PASSWORD")
	if err != nil {
		return nil, err
	}
...
func requireEnv(name string) (string, error) {
	...
	if value == "" {
		return "", fmt.Errorf(
			"[applink] missing required environment variable %s\n"+
...
func (c *Config) RequireEndpoint(service string) (string, error) {
	...
	if !known {
		return "", fmt.Errorf("[applink] unknown service %q", service)
	}
	url, configured := c.Endpoints[service]
	if !configured {
		return "", fmt.Errorf(
			"[applink] %s is not configured. Either the API is not enabled on your "+
				"application in the Applink developer portal, or %s is missing from the environment. "+
...
func (c *Config) Describe() string {
	return fmt.Sprintf(
		"applink{applicationId=%s password=***redacted*** enabledServices=%v}",
