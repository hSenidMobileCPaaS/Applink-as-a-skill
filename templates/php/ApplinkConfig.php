 ...
 *
 * Laravel: bind this in a service provider and read from config('services.applink')
...
final class ApplinkConfig
{
    ...
    public static function fromEnvironment(): self
    {
        ...
        return new self(
            self::requireEnv('APPLINK_APP_ID'),
            self::requireEnv('APPLINK_PASSWORD'),
            $endpoints,
        );
    }
    ...
    public function requireEndpoint(string $service): string
    {
        if (!isset(self::ENDPOINT_VARS[$service])) {
            throw new RuntimeException("[applink] Unknown service '{$service}'");
        }
