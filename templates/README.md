...
Working reference implementations of an Applink integration. **They are a specification, not
a framework to impose.** Read the one closest to the host project, port the structure, and keep
that project's own conventions.

Applink is JSON over HTTPS: no language, framework or runtime is privileged. Pick by what the
...
| **TypeScript / Node** | [typescript/](typescript/) — `applink-config.ts`, `applink-client.ts`, `applink-types.ts`, `callbacks-nextjs.ts`, `ussd-session.ts` | The most complete set, with the fullest commentary. Callback routes are Next.js App Router; the logic ports to Express, Fastify, Hono or NestJS unchanged |
| **Python** | [python/](python/) — `applink_config.py`, `applink_client.py`, `callbacks_fastapi.py`, `ussd_session.py` | Standard library only; `httpx`/`requests` swap noted inline. Callbacks are FastAPI; Django/Flask notes at the foot of the file |
| **Java** | [java/](java/) — `ApplinkConfig.java`, `ApplinkClient.java`, `ApplinkCallbackController.java` | Java 17 `HttpClient` + Jackson. Callbacks are Spring Boot with `@Async` |
...
| **PHP** | [php/](php/) — `ApplinkConfig.php`, `ApplinkClient.php`, `callbacks.php` | cURL extension, no Composer dependency. Framework-neutral callbacks with Laravel notes inline |
| **C# / .NET** | [csharp/](csharp/) — `ApplinkOptions.cs`, `ApplinkClient.cs`, `ApplinkCallbacks.cs` | `IHttpClientFactory` + `System.Text.Json`. Callbacks are minimal APIs with a `BackgroundService` worker |
...
- **One `tel:` normaliser** producing `tel:880…` with no `+`, and one `maskAddress()` for logs.
- **`P1003` is treated as pending, not success.** `startCharge()` returns it so the caller can
  persist `requestCorrelator` and keep the ledger row open.
- **Charging is two functions** — `startCharge()` then `confirmCharge()` — and neither retries
  internally, because a timeout does not mean the charge failed and re-running the first one
  begins a second charge.
- **Register and unregister read `subscriptionStatus`**, because Applink publishes no benign
  duplicate-state codes. Each template ships a `hasSubscriptionStatus()` helper for it.
...
- **The subscriber notification's `password` field is stripped** before the payload can reach a
  log, a trace or a queue.

## What each template deliberately leaves to you

The persistence layers, because they belong to your architecture rather than to Applink:
...
| Charge ledger (`externalTrxId` + `requestCorrelator`) | not shipped | Your database, written **before** the first charging call |
| OTP session (`referenceNo`) | not shipped | Your session store, server-side only, 5-minute TTL |
